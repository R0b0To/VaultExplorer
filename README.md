...
- **Item vault** — passwords, cards, bank accounts, notes, identities, and licenses stored as encrypted entries, like a password manager built into the container. Import from or export to KeePass (.kdbx), Bitwarden (.json), or CSV (Bitwarden, LastPass, Chrome, 1Password, Dashlane, NordPass, Apple Passwords) from the Tools tab — see [`docs/password-interchange.md`](docs/password-interchange.md)
...
- **Import / Export Passwords** — move the Item Vault's logins, cards, identities, notes, bank accounts, and software licenses to or from KeePass (.kdbx), Bitwarden (.json), or CSV. Runs entirely on-device; see [`docs/password-interchange.md`](docs/password-interchange.md) for the exact field mapping and format support
