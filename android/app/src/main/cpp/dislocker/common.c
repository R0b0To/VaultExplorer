...
/* For pread(2)/pwrite(2) prototypes -- same reason sectors.c already
 * defines this before its own <unistd.h> include. Must come before any
 * system header. */
...
int dis_close(int fd)
{
	...
	if(dis_virtual_io_is_virtual(fd))
		return dis_virtual_io_close(fd);
...
ssize_t dis_read(int fd, void* buf, size_t count)
{
	...
	if(dis_virtual_io_is_virtual(fd))
	{
		res = dis_virtual_io_read(fd, buf, count);
		if(res < 0)
			dis_printf(L_ERROR, DIS_XREAD_FAIL_STR " #%d (virtual)\n", fd);
		return res;
	}
...
ssize_t dis_write(int fd, void* buf, size_t count)
{
	...
	if(dis_virtual_io_is_virtual(fd))
	{
		res = dis_virtual_io_write(fd, buf, count);
		if(res < 0)
			dis_printf(L_ERROR, DIS_XWRITE_FAIL_STR " #%d (virtual)\n", fd);
		return res;
	}
...
off_t dis_lseek(int fd, off_t offset, int whence)
{
	...
	if(dis_virtual_io_is_virtual(fd))
	{
		res = dis_virtual_io_lseek(fd, offset, whence);
		if(res == (off_t) -1)
			dis_printf(L_ERROR, DIS_XSEEK_FAIL_STR " #%d (virtual)\n", fd);
		return res;
	}
...
/**
 * pread(2) wrapper -- VaultExplorer addition, not upstream. Used by
 * inouts/sectors.c's read_decrypt_sectors()/write_encrypt_sectors()/
 * fix_read() instead of calling pread(2)/pwrite(2) directly, so those
 * call sites get virtual-fd support without needing their own checks.
 *
 * @param fd The file (or fake fd) to read from
 * @param buf The buffer where to put read data
 * @param count The number of bytes to read
 * @param offset The offset to read at (does not disturb any lseek-tracked
 * position, same atomicity guarantee as the real pread(2))
 * @return The number of bytes read, or -1 on error
 */
ssize_t dis_pread(int fd, void* buf, size_t count, off_t offset)
{
	if(dis_virtual_io_is_virtual(fd))
	{
		ssize_t res = dis_virtual_io_pread(fd, buf, count, offset);
		if(res < 0)
			dis_printf(L_ERROR, DIS_XREAD_FAIL_STR " #%d (virtual)\n", fd);
		return res;
	}
...
/**
 * pwrite(2) wrapper -- VaultExplorer addition, not upstream. See
 * dis_pread() above.
 */
ssize_t dis_pwrite(int fd, const void* buf, size_t count, off_t offset)
{
	if(dis_virtual_io_is_virtual(fd))
	{
		ssize_t res = dis_virtual_io_pwrite(fd, buf, count, offset);
		if(res < 0)
			dis_printf(L_ERROR, DIS_XWRITE_FAIL_STR " #%d (virtual)\n", fd);
		return res;
	}
