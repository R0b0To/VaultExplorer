	...
	switch(opt_name)
	{
		...
		case DIS_OPT_INITIALIZE_STATE:
			*opt_value = (void*) cfg->init_stop_at;
			break;
		case DIS_OPT_SET_VIRTUAL_IO:
			*opt_value = (void*) cfg->virtual_io;
			break;
	}
...
int dis_setopt(dis_context_t dis_ctx, dis_opt_e opt_name, const void* opt_value)
{
	...
	switch(opt_name)
	{
		...
		case DIS_OPT_INITIALIZE_STATE:
			...
			break;
		case DIS_OPT_SET_VIRTUAL_IO:
			/* Not owned/copied -- see config.priv.h's field comment */
			cfg->virtual_io = (const dis_virtual_io_t*) opt_value;
			break;
	}
