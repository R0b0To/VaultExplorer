...
typedef enum {
	...
	/*
	 * VaultExplorer addition, not upstream: supply a dis_virtual_io_t*
	 * (see dislocker/virtual_io.h) instead of DIS_OPT_VOLUME_PATH, for
	 * volumes with no real filesystem path. Mutually exclusive with
	 * DIS_OPT_VOLUME_PATH -- whichever is set last at dis_initialize()
	 * time wins (virtual_io takes priority if both are non-NULL).
	 */
	DIS_OPT_SET_VIRTUAL_IO
} dis_opt_e;
