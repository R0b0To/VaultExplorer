...
typedef struct _dis_cfg {
	...
	/*
	 * VaultExplorer addition, not upstream. When set, dis_initialize()
	 * registers this with virtual_io.c and uses the resulting fake fd
	 * instead of dis_open()-ing volume_path. See DIS_OPT_SET_VIRTUAL_IO.
	 * Not owned by this struct -- the caller's pointer must outlive the
	 * dis_context_t (or at least outlive dis_close()/dis_destroy()).
	 */
	const dis_virtual_io_t* virtual_io;
