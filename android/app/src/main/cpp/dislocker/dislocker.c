...
int dis_initialize(dis_context_t dis_ctx)
{
	 ...
	 */
	if(!dis_ctx->cfg.volume_path && !dis_ctx->cfg.virtual_io)
	{
	...
	if(dis_ctx->cfg.virtual_io)
	{
		/*
		 * VaultExplorer addition, not upstream: the volume has no real
		 * filesystem path (e.g. it's reachable only through this app's
		 * USB sector-transport JNI calls) -- register the caller-supplied
		 * virtual I/O backend and use the resulting fake fd everywhere a
		 * real fd would otherwise be used. See virtual_io.h.
		 */
		dis_printf(L_DEBUG, "Using caller-supplied virtual I/O backend (no path)\n");
		dis_ctx->fve_fd = dis_virtual_io_register(dis_ctx->cfg.virtual_io);
		if(dis_ctx->fve_fd == 0)
		{
			dis_printf(L_CRITICAL, "Failed to register virtual I/O backend (no free slot)\n");
			dis_destroy(dis_ctx);
			return DIS_RET_ERROR_FILE_OPEN;
		}
		if(!dis_ctx->cfg.virtual_io->write)
