...
class CryfsBlockStore(
    ...
    private val fileCache = ConcurrentHashMap<String, DocumentFile>()
    private val versionCache = ConcurrentHashMap<String, Long>()

    // Memory cache for decrypted block payloads (post-integrity header)
    private val decryptedCache = object : LruCache<String, ByteArray>(512) {
        override fun sizeOf(key: String, value: ByteArray): Int = 1
    }

    private fun shardDir(prefix: String, create: Boolean): DocumentFile? {
        shardDirCache[prefix]?.let { return it }
        val existing = saf.childOf(blocksRoot, prefix)
        if (existing != null) {
            shardDirCache[prefix] = existing
            return existing
        }
        if (!create) return null
        val created = saf.createDirectorySafe(blocksRoot, prefix)
        shardDirCache[prefix] = created
        return created
    }

    fun exists(id: CryfsBlockId): Boolean {
        if (decryptedCache.get(id.hex) != null) return true
        val dir = shardDir(id.shardDir, create = false) ?: return false
        return fileCache.containsKey(id.hex) || saf.childOf(dir, id.fileName) != null
    }

    fun load(id: CryfsBlockId): ByteArray? {
        decryptedCache.get(id.hex)?.let { return it.copyOf() }

        val dir = shardDir(id.shardDir, create = false) ?: return null
        val file = fileCache[id.hex]
            ?: saf.childOf(dir, id.fileName)?.also { fileCache[id.hex] = it }
            ?: return null
    ...
    /** Encrypts and writes [payload] as block [id], creating or overwriting the block file properly. */
    fun store(id: CryfsBlockId, payload: ByteArray) {
        val dir = shardDir(id.shardDir, create = true)
            ?: throw IllegalStateException("Could not create/access block shard directory ${id.shardDir}")

        val targetFile = fileCache[id.hex]
            ?: saf.childOf(dir, id.fileName)?.also { fileCache[id.hex] = it }
            ?: saf.createFileSafe(dir, "application/octet-stream", id.fileName)?.also { fileCache[id.hex] = it }
            ?: throw IllegalStateException("Could not create/open block file ${id.hex}")

        if (!versionCache.containsKey(id.hex)) {
            load(id) // Read version if the block already existed
        }

        val version = (versionCache[id.hex] ?: 0L) + 1
        versionCache[id.hex] = version
        ...
        saf.writeWhole(targetFile, onDisk)
        decryptedCache.put(id.hex, payload.copyOf())
    }
    ...
    fun remove(id: CryfsBlockId): Boolean {
        ...
        val dir = shardDir(id.shardDir, create = false) ?: return false
        val file = fileCache.remove(id.hex) ?: saf.childOf(dir, id.fileName) ?: return false
        return file.delete()
    }
    ...
    companion object {
        ...
        const val INTEGRITY_HEADER_SIZE = 2 + 16 + 4 + 8 // 30 bytes
        ...
        fun calculateVirtualBlockSize(physicalBlockSize: Int, cipherName: String): Int {
            val cipherOverhead = when (cipherName) {
                "xchacha20-poly1305" -> 40 // 24-byte nonce + 16-byte tag
                "aes-256-gcm", "aes-128-gcm" -> 32 // 16-byte IV + 16-byte tag
                "aes-256-cfb", "aes-128-cfb" -> 16 // 16-byte IV
                else -> 40
            }
